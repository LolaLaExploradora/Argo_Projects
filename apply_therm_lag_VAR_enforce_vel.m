function apply_therm_lag_VAR_interp_t(ncfile, cyc_ii, vel_vec, dt_vec, dt_vec_juld, dpres_vec, dpres_m_vec)
...
vel_static = .1;
e_time_orig = sw_dpth(f.pres',lat(1))/vel_static;
e_time_orig = e_time_orig(ref)-e_time_orig'; % rise time


%% Rtraj data - dpres and dt
% pressure
pres_rt_m = sw_dpth(dpres_vec{cycle_number(1)+1}, lat(1));
% time
t_rt = dt_vec_juld{cyc_ii,1}; % use just the julian day (jstart for the beginning of the profile) 
int_day = floor(t_rt(1));
t_fracDay = t_rt-int_day;
t_rt_sec = t_fracDay*86400; %relative time wrt day, in sec

%% Rtraj velocity
vel_rt = diff(pres_rt_m)./diff(t_rt)

%% Profile data - pressure
pres_p_m = sw_dpth(flip(f.pres(ok)'), lat(1)); %flip pres so deep-->shallow
...
both_p_vecs{1} = pres_p_m;
both_p_vecs{2} = pres_rt_m;
for idx = 1:length(both_p_vecs)
    ...
    if idx == 2  
        t_rt_sec = t_rt_sec(~mask_strct_incr);
        t_rt_sec = t_rt_sec(mask_rmv_dupl);
    end
end
pres_p_m_msk = updated_p_vecs{1};
pres_rt_m_msk = updated_p_vecs{2};
...
pres_p_m_inv = find(track_masks{1,1}==1);
if ~isempty(pres_p_m_inv)
    ...
    %in the statement.
    b4_inv = pres_p_m_inv-1;
    presb4inv = pres_p_m(b4_inv);
    presatinv = pres_p_m(pres_p_m_inv);
    ...
    fprintf("Pressure inversion found at index/indices %d of %d (where pres vec is ascending)/n ..." + ...
       "with relative depth shift(s): %d", pres_p_m_inv, length(pres_p_m), rela_depth_shift)
end
...
v_mpers1 = diff(pres_p_m_msk)./diff(vt_sec);

%% Recreate Pelle's rise time plot
vert_vel = diff(pres_rt_m_msk)./diff(t_rt_sec);
figure
plot([vert_vel;0]*-1, pres_rt_m_msk, 'b-')
...
figure
plot( [t_rt_sec-t_rt_sec(1)], pres_rt_m_msk, 'k+-')
set(gca,'ydir','rev'); hold on
...
hold on
yyaxis right; 
plot(pres_rt_m_msk, [vert_vel;0], 'r*'); hold on
plot(pres_p_m(2:end), v_mpers1, '.-', 'Color',[1 0.7 0.4], 'LineWidth',2)
%ylim([-.06 -.15]) 
ylabel('w [m/s]')
legend('pres vs time, Rtraj', 'pres vs vel, Rtraj', 'pres vs vel, Prof')



%% Elapsed time 
intermed = diff(pres_p_m_msk)./(v_mpers1);
nanmask1 = find(isnan(intermed));
e_time0 = cumsum(diff(pres_p_m_msk)./(v_mpers1), 'omitnan');
...
%% Plot pres vs elapsed time 
figure
plot(t_rt_sec-t_rt_sec(1), pres_rt_m_msk, 'k+-'); hold on
plot(e_time_orig(end:-1:1), pres_p_m, 'b+'); hold on
plot(e_time1(end:-1:1), pres_p_m(1:end-1), 'm+')
set(gca,'ydir','rev'); hold on
xlabel('time [sec]'); ylabel('pressure [m]')
legend('Rtraj elapsed time', 'const vel elapsed time', 'VAR vel elapsed time')
...
salt_cor0=fliplr(celltm_sbe41(f.psal(ok(nok:-1:1)), temp_68(ok(nok:-1:1)), ...
    f.pres(ok(nok:-1:1)), e_time_orig(ok(nok:-1:1)), ...
    alph,tau));
